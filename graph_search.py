...
class Graph:
    ...
    def strongly_connected_component(self):
        ...
        # Otherwise, we get a pretty neat exception while processing large files
        # "RuntimeError: dictionary changed size during iteration".
        for node in self.adj_list.keys():
            if node not in visited:
                dfs_util(node)

        # Graph transpose can be done while reading the file for optimisation.
