...
void bfs(ll x,map<ll, vector< ll > > &adj_list, ll y){
	queue <ll> nodes;
	...
	while(!nodes.empty()){
		...
		tr(adj_list[v], it){
			if(!visit[*it]){
				visit[*it] = true;
				val[*it] += y;	// update the node values
				Path_sum[*it] = Path_sum[v] + val[*it];
				nodes.push(*it);
			}
		}
	}
