...
{  
	sync_false;
	ll T,N,NC;
	long double A,B,C,slope,cons;
	set <long double>cs;
	map< long double, set< long double > > slope_to_c;
	set< long double > perp;
	Scan_f(&T);
	wl(T){
		Scan_f(&N);
		NC = N;
		wl(NC){
				...
				if (N>1)
				{
					if(B){
						...
						slope = A/B;
						cons = C/B;
						if ( slope_to_c.find(slope) == slope_to_c.end() ){
		...
		if (N>1)
		{
				...
				m = max(m,sz(perp))==1?1:max(m,sz(perp));
				cpfn(m);
				slope_to_c.clear();
				perp.clear();
				cs.clear();
